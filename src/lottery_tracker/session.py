...
def standard_slots(spec=(("A", 24), ("B", 24))) -> list:
    """Build the ordered box labels, e.g. A1..A24 then B1..B24 (48 boxes)."""
    out = []
...
class CountSession:
    ...
    def commit(self, path) -> ScanLog:
        """Append every recorded scan (in box order) to the durable log."""
        if self.committed:
            raise RuntimeError("this count session was already committed")
        log = load_scans(path)
        for scan in self.scans_in_order():
            log.add(scan)
        save_scans(path, log)
        self.committed = True
        return log
